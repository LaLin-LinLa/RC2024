...
void ENCODER_loop(void)
{
	...
	//龙伯格观测器PLL
	//
	Encoder.pos_cpr_counts += CURRENT_MEASURE_PERIOD * Encoder.vel_estimate_counts;		//位置估计
	float delta_pos_cpr_counts = (float)(Encoder.count_in_cpr - (int)Encoder.pos_cpr_counts);		//位置估计增量
	...
	Encoder.phase = (interpolated_enc * M_2PI * UsrConfig.motor_pole_pairs) / ((float)Encoder.Encoder_cpr);	//电角度=机械角度*极对数
