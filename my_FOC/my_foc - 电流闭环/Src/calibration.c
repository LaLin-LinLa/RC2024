        ...
        case CS_ENCODER_CW_LOOP:
            if(sample_count < (UsrConfig.motor_pole_pairs * SAMPLES_PER_PPAIR)){
                if(time > next_sample_time){
                    ...
                    int count_ref = (phase_set * ((float)Encoder.Encoder_cpr)) / (M_2PI * (float)UsrConfig.motor_pole_pairs);
        ...
        case CS_ENCODER_END:
            {
                ...
                int lut_offset = p_error_arr[0] * OFFSET_LUT_NUM / Encoder.Encoder_cpr;
